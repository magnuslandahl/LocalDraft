...
namespace LocalDraft.App.Tests;
...
public sealed class ProjectConfigurationTests
{
    ...
    public void AppProject_UsesRequiredPortableSettings()
    {
        ...
        var project = File.ReadAllText(Path.Combine(root, "src", "LocalDraft.App", "LocalDraft.App.csproj"));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "LocalDraftAppTests", Guid.NewGuid().ToString("N"));
        try
        {
            var paths = new LocalDraft.Infrastructure.AppPathService(root);
            var repository = new LocalDraft.Infrastructure.DocumentRepository(paths);
            var viewModel = new LocalDraft.App.MainWindowViewModel(
                repository,
                new LocalDraft.Infrastructure.VersionService(paths));
    ...
    {
        var root = FindRepositoryRoot();
        var xaml = File.ReadAllText(Path.Combine(root, "src", "LocalDraft.App", "MainWindow.xaml"));
    ...
    {
        var root = FindRepositoryRoot();
        var mainXaml = File.ReadAllText(Path.Combine(root, "src", "LocalDraft.App", "MainWindow.xaml"));
        var recordingXaml = File.ReadAllText(Path.Combine(root, "src", "LocalDraft.App", "RecordingWindow.xaml"));
        var recordingCode = File.ReadAllText(Path.Combine(root, "src", "LocalDraft.App", "RecordingWindow.xaml.cs"));
        var settingsXaml = File.ReadAllText(Path.Combine(root, "src", "LocalDraft.App", "SettingsWindow.xaml"));
    ...
    {
        var root = FindRepositoryRoot();
        var path = Path.Combine(root, "src", "LocalDraft.App", "MainWindow.xaml");
    ...
    {
        var root = FindRepositoryRoot();
        var appXaml = File.ReadAllText(Path.Combine(root, "src", "LocalDraft.App", "App.xaml"));
        var startup = File.ReadAllText(Path.Combine(root, "src", "LocalDraft.App", "App.xaml.cs"));
